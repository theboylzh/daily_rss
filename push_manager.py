...
class PushManager:
    ...
    def send_weekly_analysis(self, analysis: Dict[str, Any]):
        """发送每周分析报告"""
        if not analysis:
            print("无周分析报告可发送")
            return False
        ...
        # 生成邮件内容
        subject = f"每周新闻分析报告 - {analysis['week']}"
        html_content = self._generate_weekly_html_content(analysis)
    ...
    def send_monthly_analysis(self, analysis: Dict[str, Any]):
        """发送每月分析报告"""
        if not analysis:
            print("无月分析报告可发送")
            return False
        ...
        # 生成邮件内容
        subject = f"每月新闻分析报告 - {analysis['month']}"
        html_content = self._generate_monthly_html_content(analysis)
    ...
    def _generate_weekly_html_content(self, analysis: Dict[str, Any]) -> str:
        """生成每周分析报告的HTML内容"""
        html = """
        <!DOCTYPE html>
        <html lang="zh-CN">
        ...
        # 转换Markdown为HTML
        summary_html = self._markdown_to_html(analysis['summary'])
        
        # 替换模板变量
        html = html.replace('{{week}}', analysis['week'])
        html = html.replace('{{news_count}}', str(analysis['news_count']))
    ...
    def _generate_monthly_html_content(self, analysis: Dict[str, Any]) -> str:
        """生成每月分析报告的HTML内容"""
        html = """
        <!DOCTYPE html>
        <html lang="zh-CN">
        ...
        # 替换模板变量
        html = html.replace('{{month}}', analysis['month'])
        html = html.replace('{{news_count}}', str(analysis['news_count']))
        html = html.replace('{{summary}}', summary_html)
        html = html.replace('{{timestamp}}', analysis['timestamp'])
